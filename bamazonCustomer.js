      ...
      connection.query(
        ...
        function (err, data) {
          ...
          if (itemInfo.length === 0) {
            console.log(
              "Error: Product does not exist. Please select a valid Product ID."
            );
            showItems();
          } else {
            if (quantity <= itemInfo.stock_quantity) {
              var updateItem =
                ...
                (itemInfo.stock_quantity - quantity) +
                " WHERE item_id = " +
                answer.item_id;
              ...
              connection.query(updateItem, function (err, data) {
                if (err) throw err;
                console.log(
                  "Your total amount spent is $" + itemInfo.price * quantity
                );
                connection.end();
              });
            } else {
              console.log(
                "Unfortunately, we do not have enough of that product in stock, please try a different product."
              );
              showItems();
            }
          }
